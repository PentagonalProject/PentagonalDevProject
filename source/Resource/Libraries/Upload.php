...
class CI_Upload {
    ...
    public function do_upload($field = 'userfile')
    {
        ...
        if ( ! isset($_file))
        {
            $this->set_error('You did not select a file to upload.', 'debug');
            return FALSE;
        }
        ...
        if ( ! is_uploaded_file($_file['tmp_name']))
        {
            ...
            switch ($error)
            {
                case UPLOAD_ERR_INI_SIZE:
                    $this->set_error('The uploaded file exceeds the maximum allowed size in your PHP configuration file.', 'info');
                    break;
                case UPLOAD_ERR_FORM_SIZE:
                    $this->set_error('The uploaded file exceeds the maximum size allowed by the submission form.', 'info');
                    break;
                case UPLOAD_ERR_PARTIAL:
                    $this->set_error('The file was only partially uploaded.', 'debug');
                    break;
                case UPLOAD_ERR_NO_FILE:
                    $this->set_error('You did not select a file to upload.', 'debug');
                    break;
                case UPLOAD_ERR_NO_TMP_DIR:
                    $this->set_error('The temporary folder is missing.', 'error');
                    break;
                case UPLOAD_ERR_CANT_WRITE:
                    $this->set_error('The file could not be written to disk.', 'error');
                    break;
                case UPLOAD_ERR_EXTENSION:
                    $this->set_error('The file upload was stopped by extension.', 'debug');
                    break;
                default:
                    $this->set_error('You did not select a file to upload.', 'debug');
                    break;
        ...
        {
            $this->set_error('The filetype you are attempting to upload is not allowed.', 'debug');
            return FALSE;
        }
        ...
        {
            $this->set_error('The file you are attempting to upload is larger than the permitted size.', 'info');
            return FALSE;
        }
        ...
        {
            $this->set_error('The image you are attempting to upload doesn\'t fit into the allowed dimensions.', 'info');
            return FALSE;
        }
        ...
        if ( ! @copy($this->file_temp, $this->upload_path.$this->file_name))
        {
            ...
            {
                $this->set_error('A problem was encountered while attempting to move the uploaded file to the final destination.', 'error');
                return FALSE;
            }
        }
    ...
    public function set_filename($path, $filename)
    {
        ...
        if ($new_filename === '')
        {
            $this->set_error('The file name you submitted already exists on the server.', 'debug');
            return FALSE;
        }
        else
        {
    ...
    public function is_allowed_filetype($ignore_mime = FALSE)
    {
        ...
        {
            $this->set_error('You have not specified any allowed file types.', 'debug');
            return FALSE;
        }
    ...
    public function validate_upload_path()
    {
        if ($this->upload_path === '')
        {
            $this->set_error('The upload path does not appear to be valid.', 'error');
            return FALSE;
        }
        ...
        {
            $this->set_error('The upload destination folder does not appear to be writable.', 'error');
            return FALSE;
        }
